...
```{r}
# Create list of models to compare
model_list <- list(model2a = model2a, model2b = model2b, model2c = model2c)
...
The model using `txti2` alone performed better at predicting disease progression than the model using `kidvol_base` alone (AUC = , 95% CI: _,_). However, this difference was not statistically significant (p = ___).
...
# Plot ROC Curves on Top of each other here.
