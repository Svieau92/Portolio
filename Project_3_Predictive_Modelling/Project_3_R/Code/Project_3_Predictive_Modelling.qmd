...
```{r}
# Z-score scale variables
data1_train <- data1_train |> 
  mutate(geom_avg_scale = scale(geom_avg),
         gabor3_scale = scale(gabor3),
         txti2_scale = scale(txti2),
         lbp5_log_scale = scale(lbp5_log),
         glcm2_square = glcm2**2,
         glcm2_square_scale = scale(glcm2_square))
```
...
The cost function in a linear regression is Mean Square Error (MSE):
...
The goal is to select a mapping function that minimizes the cost function and thereby produces the best predictions of the outcome variable.
...
# Performing the Predictive Model

[This website](http://zevross.com/blog/2017/09/19/predictive-modeling-and-machine-learning-in-r-with-the-caret-package/) used for coding guidance

## Model 1A: Baseline Kidney Volume

To answer the researcher's first question, we will run a predictive model that uses the baseline height-corrected total kidney volume to predict percent change in kidney volume.

$$ Kidney Volume Change = 𝛽_0 + 𝛽_{Log Kidney Volume Baseline} + e $$

We will be using the `caret` package in R to perform the linear regression using 5-fold cross validation.
...
the RMSE of the model including `kidvol_base` alone is8.11
...
## Model 2: MRI Features

To answer the researcher's second question, we will run a predictive model that uses only MRI image features to predict percent change in kidney volume at year 3.

$$ Kidney Volume Change = 𝛽_0 + 𝛽_{Image Feature 1} + 𝛽_{Image Feature 2}+ ... + e $$
...
# Perform the linear regression using 5-fold CV
model1b <- train(kidvol_change ~ gabor3_scale + txti2_scale + glcm2_square_scale, 
                 data = data, 
                 method = "lm",
                 trControl = tc)
# Get RMSE
...
Our residuals are smaller in this model compared to modela 1A using `kidvol_base` alone.
...
# Perform the linear regression using 5-fold CV
model1b2 <- train(kidvol_change ~ txti2_scale, 
                 data = data, 
                 method = "lm",
                 trControl = tc)
...
The RMSE is now 7.89, slightly lower than with `kidvol_base_log` alone!

This model predicts 17.8% of the variance in `kidvol_change`.
...
The predicted models are still not that class to the actual values.
...
The residuals are approximately normally distributed presumably they would be with a larger sample size.
...
#### Baseline Kidney Volume and MRI Features
...
$$ Kidney Volume Change = 𝛽_0 + 𝛽_{Baseline Kidney Volume} + 𝛽_{Txti2} +  e $$
...
summary(model1c)
```
The RMSE for the combined model is 7.84, about the same as the model using just `txti2` alone.
...
ggplot(data, aes(sample = residuals1c)) +
  ...
  geom_qq_line() +
  theme_minimal() +
  labs(title = "QQ Plot of Baseline Kidney Volumne and  TXTI2")
```
...
model_list <- list(model1a = model1a, model1b = model1b2, model1c = model1c)
...
# Train the linear regression model with 5-fold cross-validation
model <- train(mpg ~ ., data = mtcars, method = "lm", trControl = control)

# Extract resample results
resamples <- model$pred

# Get the final model coefficients
final_model <- model$finalModel

# Predict using the final model
predictions <- predict(final_model, newdata = mtcars)

# Calculate prediction intervals manually
pred_intervals <- predict(final_model, newdata = mtcars, interval = "prediction")

# Combine actual values, predictions, and intervals
results <- data.frame(
  Actual = mtcars$mpg,
  Fit = predictions,
...
ggplot(prediction_data, aes(x = Actual, y = Fit)) +
  geom_point() +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2, color = "blue") +
  labs(title = "Predictions with Error Bars (Linear Regression)", 
...
```{r}
compare_models(model1a, model1b2)
compare_models(model1a, model1c)
compare_models(model1c, model1b2)
```

Model performance is not statistically significant from each other. This may be due to low sample size however.


#### Conclusion

The model with the lowest RMSE is the model 
