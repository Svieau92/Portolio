...
# Use interactions package to plot interaction
cat_plot(model_MENT_main1, pred = hard_drugs_grp, modx = ADH_HIGHVSLOW, geom = "line", colors = "Pastel2")
```
...
```{r}
# Use interactions package to plot interaction
cat_plot(model_PHYS_final1, pred = hard_drugs_grp, modx = ADH_HIGHVSLOW, geom = "line", colors = "Pastel2")
```
...
# Crazy interaction with depression and hard drugs and mental qol
```{r}
# Create a scatterplot of mental QOLchange by depression score, colored by hard drug use group.
ggplot(data_wide_2, aes(x = CESD_2, y = AGG_MENT_CHANGE, color = hard_drugs_grp)) + 
  geom_point() +
  ...
  scale_fill_brewer(palette = "Pastel2")
```
If you were a previous hard drug user, increases in depression score was associated with decreases in mental QOL over 2 years.

```{r}
# Create a scatterplot of mental QOLchange by depression score, colored by hard drug use group.
ggplot(data_wide_2, aes(x = AGG_MENT_CHANGE, y = CESD_2, color = hard_drugs_grp)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal() +
  labs(title = "Mental QOL Change by Depression at 2 years, Colored by Hard Drug Use Group",
  ...
  scale_fill_brewer(palette = "Pastel2")
```
If you were a previous hard drug user, increases in mental QOL predicted lower depression scores over 2 years.

They are saying the same thing.

Except this plot is showing that never drug users were less depressed compared to curent users, regardless of whether they increased or decreased in mental QOL.

So mental QOL increase is protective for previous drug users in particular.

```{r}
# Create a scatterplot of mental QOLchange by depression score, colored by hard drug use group.
ggplot(data_no_na, aes(x = CESD_2, y = AGG_MENT_CHANGE, color = ADH_HIGHVSLOW)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal() +
  labs(title = "Mental QOL Change by Depression at 2 years, Colored by Adherence",
       y = "Mental QOL Change",
       x = "Depression Score") + 
  scale_fill_brewer(palette = "Pastel2")
...
It appears that high adherence is protective for those with high depression scores.

I think there's a three way interaction going on here.




## Depression and leu3n change are mediators for QOL score
...
# Perform regression on mental QOL change by hard drugs group, full model
model_MENT_full1 <- lm(AGG_MENT_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + CESD_2 + LEU3N_CHANGE +  hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)
...
## 3 way interaction with depression, hard drug use, and adherence on mental QOL
...
# Perform regression on mental QOL change by hard drugs group, full model
model_MENT_full1 <- lm(AGG_MENT_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW +  CESD_2 + hard_drugs_grp*CESD_2*ADH_HIGHVSLOW, data = data_wide_2)

# Examine the summary
summary(model_MENT_full1)
```

I do NOT want to interpret that, but that is the highest r squared i've seen by far for this data set.

Also we definitely do not have the sample size to run this.

### More stuff on possible 3-way interaction 
...
# Perform regression on mental QOL change by hard drugs group, full model
model_MENT_full1 <- lm(AGG_MENT_CHANGE ~ ADH_HIGHVSLOW + CESD_2 + ADH_HIGHVSLOW*CESD_2, data = data_wide_2)
...
```{r}
# Create a scatterplot of mental QOLchange by depression score, colored by hard drug use group.
ggplot(data_wide_2, aes(x = CESD_2, y = AGG_MENT_CHANGE, color = ADH_HIGHVSLOW)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal() +
  labs(title = "Mental QOL Change by Depression at 2 years, Colored by Hard Drug Use Group",
...
I don't actually have to look at this because we're not too interesting in depression.
...
# Create NA omitted data set
clean_data_test <- data_wide_2 %>%
  dplyr::select(AGG_MENT_CHANGE, hard_drugs_grp, CESD_2) %>%
  na.omit()

# Fit the model with X predicting M
model_mediator <- lm(CESD_2 ~ hard_drugs_grp, data = clean_data_test)

# Fit the model with X and M predicting Y
model_outcome <- lm(AGG_MENT_CHANGE ~ hard_drugs_grp + CESD_2, data = clean_data_test)

# Check for mediation
mediation_result <- mediate(model_mediator, model_outcome, treat = "hard_drugs_grp", mediator = "CESD_2")
summary(mediation_result)

```
Apparently this means that while indirect mediation is significant, the combined direct and indirect effects are not. So depression is NOT a mediate for the relationship between hard drugs and mental QOL.
