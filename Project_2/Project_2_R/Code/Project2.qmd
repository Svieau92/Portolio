...
Is depression related to CD4+ T cell count? Is it related to adherence? Do we have to control for it?
...
Oh shoot, it is! Need to go back and re-include to final model.


Is depression related to adherence?

```{r}
model <- lm(CESD_2 ~ ADH_HIGHVSLOW, data = data_wide_2)
summary(model)
```

Very significant, but a low r squared so I think we include it as a potential confounder, instead of worrying about VIF. Also should be able to run a model and get VIF output
...
Change the reference category of hard drug use group to never users, and the reference group of adherence to low adherence to compare the impact of hard drug use groups for those with low adherence to the treatment regiment.
...
```{r}
p_values <- summary(model_VLOAD_full1)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values

# Perform Bonferroni correction 
p_adjusted <- p.adjust(p_values, method = "bonferroni")

# Get the 95% CIs
conf_intervals <- confint(model_VLOAD_full1)

# Compare adjusted p-values to unadjusted p-values, with 95% CI's
p_comparison <- cbind(p_values, p_adjusted, conf_intervals)
pretty_print(p_comparison)
```
...
Change the reference group to previous drug users and low adherence.
...
```{r}
# Get p-values
p_values <- summary(model_VLOAD_full2)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values

# Perform Bonferroni correction 
p_adjusted <- p.adjust(p_values, method = "bonferroni")

# Get the 95% CIs
conf_intervals <- confint(model_VLOAD_full2)
...
```{r}
# Get p-values
p_values <- summary(model_VLOAD_full3)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values

# Perform Bonferroni correction 
p_adjusted <- p.adjust(p_values, method = "bonferroni")

# Get the 95% CIs
conf_intervals <- confint(model_VLOAD_full3)
...
The p-value is significant (p < 0.0001), indicating that the interaction term adds explanator power to the model and should be included. Thus we move on with interpretting the interaction.
...
The relationship between adherence and log viral load differed by hard drug usage.

For those with low adherence to the treatment regiment, current hard drug users had a change in log viral load that was 3.48 log copies/mL (or change in copies/mL that was 32.32 times) higher than never hard drug users (t = 2.458, p-adjusted  = 0.04284, 95% CI: 0.70 to 6.25 log copies/mL, or 2.01 to 519.78 times copies/mL). Additionally, for those with low adherence to the treatment regimen, previous hard drug users had a change in log viral load that was 5.81 log copies/mL (or change in copies/mL that was 333.05 times) greater than never hard drug user (t = 4.90, p-adjusted < 0.0001, 95% CI: 3.48 to 8.14 log copies/mL, or 32.42 to 3421.49 times copies/mL). For those with low adherence to the treatment regiment, the difference between previous and current drug users was not statistically significant (t = -1.35, p-adjusted = 0.177, 95% CI: -5.72 to 1.05 log copies/mL, or 0.0033 to 2.87 times copies/mL). 

For those with high adherence to the protocol, there was no difference in log viral load between current and never hard drug users (t = 1.920, p-adjusted = 0.388), previous and never hard drug users (t = -0.086, p-adjusted = 1.00), or previous and current hard drug users (t = 1.355, p-adjusted = 1.00).

The relationship between drug use and log viral load differed by adherence to the treatment regiment.

For current drug users, those with high adherence had a change in log viral load that was 3.53 log copies/mL less (or 0.029 times) than that of those with low adherence (t = -2.554, p-adjusted = 0.0999936, 95% CI: -6.25 to -0.70 log copies/mL, or 0.0019 to 0.50 times). This relationship was not statistically significant following Bonferroni correction however, likely due to the small number of patients who were hard drug users with low protocol adherence (n = 4). This relationship however appears to be real based on trends in the data, and we recommend increasing the sample size to discover the true differences. 

For previous drug users, those with high adherence had a change in log viral load that was 6.64 log copies/mL less (or 0.0013 times) than those with low adherence (t = -5.63, p-adjusted < 0.0001, 95% CI: -8.95 to -4.32 log copies/mL, or 0.00013 to 0.013 times).

For never drug users, the difference in log viral load between those with high adherence and low adherence was not statistically significant (t = -1.61, p-adusted = 0.759, 95% CI: -1.75 to 0.17 log copies/mL, or 0.17 to 1.19 times). 

Education at baseline is a significant predictor for change in log viral load over 2 years, while controlling for drug use and adherence to treatment regiment (t = -2.872, p = 0.00424). On average, those with a college education had an 0.85 log copies/mL (or 8.06 copies/mL) greater decrease in log viral load than those without a college education (95% CI:-1.43 to -0.27 log copies/mL).

There are additional comparisons we can make, but they are of little clinical relevance (e.g. current drug users with low adherence had a higher viral load compared to never drug users with high adherence). Basically, any comparison made between a current or previous had drug user with low adherence will be significant.
...
cat_plot(model_VLOAD_full, pred = hard_drugs_grp, modx = ADH_HIGHVSLOW, geom = "line", colors = "Pastel2")
```
...
# new_data <- expand.grid(hard_drugs_grp = data_wide_2$hard_drugs_grp,
#                         ADH_HIGHVSLOW = data_wide_2$ADH_HIGHVSLOW,
#                         EDUC_COLLEGE = data_wide_2$EDUC_COLLEGE)
# 
# new_data <- new_data %>%
#   mutate(predicted_VLOAD = predict(model_VLOAD_full1, newdata = new_data))
# 
# 
# ggplot(data_wide_2, aes(x = hard_drugs_grp, y = predicted_VLOAD, color = ADH_HIGHVSLOW, group = ADH_HIGHVSLOW)) +
#   geom_line(size = 1) +
#   scale_color_brewer(palette = "Pastel2") +
#   labs(title = "Interaction between Hard Drugs and Adherence on Predicted Viral Load",
#        x = "Hard Drugs Group",
...
We can see the main effects for adherence: for previous and current hard drug users, those with high adherence had a reduced viral load.

Similarly, we can see the main effects for hard drug use: For those with low adherence, previous and current drug users had a higher viral load compared to those with high adherence. 
...
## Make this a new Tab - Main Effects
...
```{r}
# Get p-values
p_values <- summary(model_VLOAD_red1)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values

# Perform Bonferroni correction 
p_adjusted <- p.adjust(p_values, method = "bonferroni")

# Get the 95% CIs
conf_intervals <- confint(model_VLOAD_red1)
...
This is me testing backwards selection

Let's say the full model has the interaction term between hard drug use and adherence high low

```{r}
# Perform full model with interaction term
model_log_VLOAD_full <- lm(VLOAD_log_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + EDUC_COLLEGE + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)

# Perform backwards selection model
step_model <- step(model_log_VLOAD_full, direction = "backward")
...
The candidate variables for inclusion in our model predicting `LEU3N_CHANGE` were `hard_drugs_grp`, `ADH_HIGHLOW`, `BMI_2`, `FRP_2`, and `EDUC_COLLEGE`. The researchers are interested in if differences in treatment response between the drug use groups can be explained by differences in adherence to the HAART regimen, so we will also include an interaction term between hard drug use group and adherence.
...
model_LEU3N_full <- lm(LEU3N_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + BMI_2 + FRP_2 + EDUC_COLLEGE + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)

# Exmamine the summary
summary(model_LEU3N_full)
```

BMI is significant, but has 10% missing values. Let's try running without it. (it's also correlated to college education, so we can kind if capture it with that variable).
...
# Perform regression on CD4+ T Cell count change by hard drugs group, include adherence as confounder, and BMI, frailty related phenotype, and college education as precision variables.
model_LEU3N_full <- lm(LEU3N_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + FRP_2 + EDUC_COLLEGE + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)

# Exmamine the summary
summary(model_LEU3N_full)

# Use interactions package to plot interaction
cat_plot(model_LEU3N_full, pred = hard_drugs_grp, modx = ADH_HIGHVSLOW, geom = "line", colors = "Pastel2")
```
...
# Perform regression on CD4+ T Cell count change by hard drugs group, include adherence as confounder, and BMI, frailty related phenotype, and college education as precision variables.
model_LEU3N_fullb <- lm(LEU3N_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + BMI_2 + FRP_2 + EDUC_COLLEGE, data = data_wide_2)

# Examine the summary
summary(model_LEU3N_fullb)
```
...
# Perform regression on CD4+ T Cell count change by hard drugs group, include adherence as confounder, and BMI, frailty related phenotype, and college education as precision variables.
model_LEU3N_fulla <- lm(LEU3N_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + BMI_2 + FRP_2, data = data_wide_2)

# Examine the summary
summary(model_LEU3N_fulla)

anova(model_LEU3N_fulla, model_LEU3N_fullb)
```

We get an NA for the interaction term. This is likely because dropped the n=4 patients that were current hard drug users with low adherence, making the interaction impossible to make. Indeed we can see we only have 7, 453 DF.
...
# Perform regression on CD4+ T Cell count change by hard drugs group, include adherence as confounder, and BMI, frailty related phenotype, and college education as precision variables.
model_LEU3N_full <- lm(LEU3N_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + BMI_2 + FRP_2 + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)
...
```{r}
# Gather p-values of the model
p_values <- summary(model_LEU3N_full)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values
...
```{r}
library(car)
vif_values <- vif(model_LEU3N_full)
vif_values
```
...
# Gather p-values of the model
p_values <- summary(model_LEU3N_red)$coefficients[,4] # This line selects the fourth column of the resulting coefficients table from summary(model), which is the p-values
...
model_MENT_full <- lm(AGG_MENT_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + CESD_2 + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)

# Exmamine the summary
summary(model_MENT_full)

# Use interactions package to plot interaction
cat_plot(model_MENT_full, pred = hard_drugs_grp, modx = ADH_HIGHVSLOW, geom = "line", colors = "Pastel2")
```
...
# Perform regression on mental QOL change by hard drugs group, full model
model_MENT_full <- lm(AGG_MENT_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + hard_drugs_grp*ADH_HIGHVSLOW, data = data_wide_2)
...
```{r}
data$perfADH <- ifelse(data$ADH == "100%", 1, 0)

data$perfADH <- factor(data$perfADH,
                           levels = c(0,1),
                           labels = c("Low Adherence", "PERF Adherence"))
...
# Get means for each year
summary_data <- data %>%
  group_by(perfADH, years) %>%
  summarize(Average_VLOAD_log = mean(VLOAD_log, na.rm = TRUE))

# Create plot
p <- ggplot(summary_data, aes(x = years, y = Average_VLOAD_log, color = perfADH)) +
  geom_line(size = 1) +   # Line plot
  geom_point(size = 2) +  # Add points for clarity
  labs(title = "Average VLOAD log by PERFECT Adherence Group Over 8 Years",
       x = "Year",
       y = "Average VLOAD log",
       color = "Adherence Group") +
  theme_minimal() +       # Clean theme
...
For log viral load change of beta -5.72 , do you do exp(-5.72 ) or exp(5.72) and take the negative?


THis is another (and probably easier) way of doing between group comparisons. But I noticed that the estimates are slightly off and it makes me wonder at relying on a package.
...
emm <- emmeans(model_VLOAD_full, ~ hard_drugs_grp * ADH_HIGHVSLOW)
