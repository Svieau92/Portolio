...
[Options:]{.underline} We can try running the analysis with our transformed `LEU3N` values, or run it without transformation and see how many of those high values are real outliers with high leverage and influence.
...
Now let's perform internal consistency checks to make sure those calculation were performed correctly, as well as reexamine the distributions of the change score for each variable.
...
## Hard Drug Use Groups {#Hard_Drugs}
...
table1 <- table1(~AGG_MENT_0 + AGG_PHYS_0 +  HASHV_0 + HASHF_0 + income_0 + BMI_0 + HBP_0 + DIAB_0 + LIV34_0 + KID_0 + FRP_0 + FP_0 + TCHOL_0 + TRIG_0 + LDL_0 + DYSLIP_0 + CESD_0 + SMOKE_0 + DKGRP_0 + HEROPIATE_0 + IDU_0 + LEU3N_0 +  VLOAD_log_0 + hard_drugs_grp + RACE_0 + EDUCBAS_0 + age_0 + AGG_MENT_CHANGE + AGG_PHYS_CHANGE + VLOAD_log_CHANGE + LEU3N_CHANGE| ADH_2, data = data_wide_2, caption = "Descriptive Statistics at Baseline", overall = c(left="Total"))
table1
```
...
We will first perform an exploratory data analysis to assess the relationships between variables, to examine multicollinearity, and to select potential covariates.
...
`CESD` has a strong negative associated with `AGG_MENT`, which makes perfect sense. Those who are are more depressed will have lower mental health scores. `CESD` will have to be included in any analysis with `AGG_MENT` as an outcome variable.

`FRP` and `FP` are strongly negatively associated with `AGG_PHYS`. This also makes perfect sense: Those who are frail will have lower overall physical health scores. `FRP` and/or `FP` will need to be included in the model predicting `AGG_PHYS`. Additionally, the correlation between `FRP` and `FP` will have to be assessed to see if we have multicollinearity. If so, one of them can be chosen over the other since it sounds like they are evaluating almost the same thing (i.e., "Frailty Related Phenotype" vs "Frailty Phenotype")

`income` is strongly associated with `EDUCBASE`, `RACE`, `SMOKE`, and weakly correlated with `TCHOL` and `LDL`, and `VLOAD_log`. These relationships will all have to be examined more closely to control for multicollinearity.

`ADH` is correlated with `VLOAD_log`, but I think having a categorical variable in a correlation matrix is actually not appropriate. `VLOAD_log` appears to be lightly correlated with `income` and `EDUCBASE`.

`Hard_drugs` is highly correlated with `HEROPIATE` and `IDU`, which makes perfect sense, as they are all basically the same thing. Since `Hard_drugs` is our main independent variable of interest, we drop `HEROPIATE` and `IDU` as potential covariates since they are all so highly correlated.

We appear to have notable multicollinearity between `DYSLIP` and `TCHOL`, `LDL`, and `TRIG`. This makes sense because all these variables are highly related (abnormal levels of fats in the blood). This is interesting because these variables are where a lot of our missingness occured. We will have to be careful when including any of these variables in our model.

We also have some possible multicollinearity to be aware of between `KID`, `HBP`, and `DIAB`.

With all that in mind, let's look at our relationships between the main IV and outcomes of interest!!
...
Let's begin by seeing if there appears to be a difference in our outcome variables based on hard drug use

::: panel-tabset

## Log Viral Load

Start with log viral load.
...
ggplotly(p)
```
There may be a difference between log viral load change and hard drug use group, especially between never users and previous users.
...
maybe do MI for TCHOl, or try clearing out outliers +- 3 SD from mean and re-plot hist and qqplots

turn my kable pretty print into a function that I can just call as pretty(df)
