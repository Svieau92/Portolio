...
There may be a difference between log viral load change and hard drug use group, especially between never users and previous users. Visually, never users may have had more of a decrease in log viral load over time compared to previous users and potentially current users.

Let's run the regression to see if this relationship is statistically significant.

```{r}
# Perform regression predicting log vload change by hard drug use group
...
The overall model is significant (F~(2,529)~= 3.991, p = 0.01903). The group comparisons were not significant after applying Bonferroni correction. However, they may be significant after including precision variables or confounders, and thus we continue with the analysis and include `hard_drugs_group` as the PEV.
...
The first step toward investigating this is to examine if there is a relationship between adherence and treatment response, such as log viral load change.
...
Looking at the between-group comparisons, the 100% group is statistically different from the 75-95% group (p-adjusted = 0.0008421) and the <75% group (p-adjusted = 0.0266202). 

The 75-95% group is significantly different from the 95-99% group (p-adjusted = 0.0001454).

The 100% and 95-99% group were not significantly different from each other (p > .05), and the 75-95% and <75% groups were not significantly different from each other (p-adjusted > 0.5) strong providing evidence for grouping these levels together and separating based on high vs low adherence.
...
Therefore, analyses moving forward will only make comparisons between high vs low adherence.
:::
...
Therefore, it is more prudent to collapse this variable and make comparisons only between high (95-100%) adherence and low (<95%) adherence.
...
The overall model is statistically significant (F~(1,518)~= 17.78, p = 0.00002922). On average, those in the high adherence group have a mean change in log viral load that is 1.86 times lower than those in the lower adherence group
...
`BMI` is not a significant predictor of log viral load change and should not be included in the final model.

```{r}
# Create a scatterplot of log vload change by BMI at baseline
p <- ggplot(data_wide_2, aes(x = BMI_0, y = VLOAD_log_CHANGE, color = hard_drugs_grp)) + 
  geom_point() +
  labs(title = "Log Viral Load Change by BMI at Baseline",
       y = "Log Viral Load Change",
       x = "BMI at Baseline") + 
...
That's interesting. There doesn't appear to be an overall relationship between BMI and log viral load change, but there may be an interaction there based on hard drug use group. Where low BMI recent drug users have higher log viral loads compared to those with high BMIs

Will investigate more later.

We need to double check for multicollinearity between `BMI` and `EDUC_COLLEGE`. 
...
The correlation between `BMI` and `EDUC_COLLEGE` is not significant or strong enough to be of concern. Both variables can be included in models without raising issues of multicollinearity.
...
p <- ggplot(data_wide_2, aes(x = EDUC_COLLEGE, y = VLOAD_log_CHANGE, fill = EDUC_COLLEGE)) + 
  geom_boxplot(alpha = 0.5) +
  theme_minimal() +
...
There appears to be a difference in log viral load change based on college education. Remember that this is on a logarithmic scale, so the real differences will be much more pronounced.
...
The overall model is significant F~(1,530)~= 9.204, p = 0.002533). `EDUC_COLLEGE` is a significant predictor of log viral load change and should be included in the final model.
...
We can also plot this relationship longitudinally.

```{r}
# Get means for each year
summary_data <- data_2 %>%
  group_by(hard_drugs, years) %>%
  summarize(Average_VLOAD = mean(VLOAD_log, na.rm = TRUE))

# Create plot
ggplot(summary_data, aes(x = years, y = Average_VLOAD, color = hard_drugs)) +
  geom_line(size = 1) +   # Line plot
  geom_point(size = 2) +  # Add points for clarity
  labs(title = "Average Log VLOAD by Hard Drug Use Over 8 Years",
       x = "Year",
       y = "Log VLOAD",
       color = "Hard Drug Use Since Last Visit") +
  theme_minimal() +       # Clean theme
  scale_color_brewer(palette = "Pastel2")  # Nice color palette for groups
```

This plot shows that those who used hard drugs since the last visit had potentially lower viral load... the opposite of what you would expect if drug use weakens the immune system.
...
ggplot(summary_data, aes(x = years, y = Average_LEU3N, color = ADH)) +
  geom_line(size = 1) +   # Line plot
  geom_point(size = 2) +  # Add points for clarity
  labs(title = "Average LEU3N by Adherence Group Over 2 Years",
       x = "Year",
       y = "Average LEU3Ne",
       color = "Adherence Group") +
  theme_minimal() +       # Clean theme
...
We have the same relationship here


```{r}
# Create boxplots of CD4+ T Cell Count change by adherence group
p <- ggplot(data_wide_2, aes(x = ADH_2, y = LEU3N_CHANGE, fill = ADH_2)) +
  geom_boxplot(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Adherence at Year 2",
       x = "Adherence at year 2",
       y = "CD4+ T Cell Count") +
  guides(fill = guide_legend(title = "Adherence Group"))
...
```{r}
# Create boxplots of CD4+ T Cell Count change by adherence group
p <- ggplot(data_wide_2, aes(x = ADH_HIGHVSLOW, y = LEU3N_CHANGE, fill = ADH_HIGHVSLOW)) +
  geom_boxplot(alpha = 0.5) +
  theme_minimal() +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Adherence at Year 2",
...
```{r}
# Create boxplots of CD4+ T Cell Count by hard drug use group
p <- ggplot(data_wide_2, aes(x = hard_drugs_grp, y = LEU3N_CHANGE, fill = hard_drugs_grp)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Hard Drug Use Group",
       x = "Hard Drug Use Group",
       y = "CD4+ T Cell Count Change") +
  guides(fill = guide_legend(title = "Hard Drug Use Group"))
...
There does not appear to be much of a difference in CD4+ T cell count over 2 years based on hard drug use.
...
```{r}
# Create boxplots of aggregate mental QOL by hard drug use group
p <- ggplot(data_wide_2, aes(x = hard_drugs_grp, y = AGG_MENT_CHANGE, fill = hard_drugs_grp)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of Aggregate Mental QOL by Hard Drug Use Group",
       x = "Hard Drug Use Group",
...
Lots of variation in our boxplots there, but doesn't really look like a significant difference between groups.
...
p <- ggplot(data_wide_2, aes(x = hard_drugs_grp, y = AGG_PHYS_CHANGE, fill = hard_drugs_grp)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
...
Also doesn't look like a difference here.
...
The researchers are interested in assessing adherence as a potential confounder for the relationship between hard drug use and the outcome variables. Let's assess
...
p <- ggplot(data_wide_2, aes(x = ADH_2, y = VLOAD_log_CHANGE, fill = ADH_2)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of Log Viral Load Change by Adherence",
       x = "Adherence",
       y = "Log Viral Load Change") +
...
It does appear that log viral load change differs based on adherence. It seems that the low adherence groups have a less of a decrease in log viral load compared to the high adherence groups.

If this relationship is significant we will definitely have to include adherence as a covariate.
...
```{r}
# Create boxplots of CD4+ T Cell Count by adherence group
p <- ggplot(data_wide_2, aes(x = ADH_2, y = LEU3N_CHANGE, fill = ADH_2)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Adherence",
       x = "Adherence",
       y = "CD4+ T Cell Count Change") +
  guides(fill = guide_legend(title = "Adherence"))
...
There does not appear to be much of a difference in CD4+ T Cell Count based on adherence.
...
```{r}
# Create boxplots of aggregate mental QOL change by adherence group
p <- ggplot(data_wide_2, aes(x = ADH_2, y = AGG_MENT_CHANGE, fill = ADH_2)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Adherence",
       x = "Adherence",
...
There does not appear to be much of a difference in aggregate mental QOL change based on adherence.
...
```{r}
# Create boxplots of aggregate physical QOL change by adherence group
p <- ggplot(data_wide_2, aes(x = ADH_2, y = AGG_PHYS_CHANGE, fill = ADH_2)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_brewer(palette="Pastel2") +
  labs(title = "Boxplot of CD4+ T Cell Count by Adherence",
       x = "Adherence",
...
# Final Model Log Viral Load Change

```{r}
model_log_VLOAD <- lm(VLOAD_log_CHANGE ~ hard_drugs_grp + ADH_HIGHVSLOW + EDUC_COLLEGE, data = data_wide_2)
...
This doesn't tell us anything but I want to come back to this idea.

```{r}
ggplot(data_wide_2, aes(x = newid, y = VLOAD_log_CHANGE, color = hard_drugs_grp)) +
  geom_point()
```
