...
The relationship between adherence to the treatment regiment and change in log viral load over 2 yeras differed by hard drug use.
...
## Descriptive Statistics Filter

Filtered table 1 based on likely variables for end models, dropping extraneous variables.

```{r}
# Create Table 1 for the wideform data set,stratified by hard drug use group
table1 <- table1(~AGG_MENT_0 + AGG_PHYS_0 + BMI_0 + CESD_0 + DKGRP_0 + LEU3N_0 +  VLOAD_log_0 + ADH_HIGHVSLOW + RACE_WHITEYN + EDUC_COLLEGE + age_0 + AGG_MENT_CHANGE + AGG_PHYS_CHANGE + VLOAD_log_CHANGE + LEU3N_CHANGE| hard_drugs_grp, data = data_wide_2, caption = "Descriptive Statistics at Baseline", overall = c(left="Total"))
table1
```
______________
